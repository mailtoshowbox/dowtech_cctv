...
class ModelUserUser extends Model {
	...
	public function getUsers($data = array()) {
		$sql = "SELECT * FROM `" . DB_PREFIX . "user` WHERE user_id <> 1 ";
	...
	public function getTotalUsers() {
		$query = $this->db->query("SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "user`  WHERE user_id <> 1 ");

		return $query->row['total'];
	}

	public function getTotalUsersByGroupId($user_group_id) {
		$query = $this->db->query("SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "user`    WHERE   user_group_id <> 1 AND  user_group_id = '" . (int)$user_group_id . "'");
